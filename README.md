...
- 平台工作台：顶部切换 `抖音` 或 `Bilibili`；切换时清空结果，拒绝执行不属于当前平台的链接。
- 抖音：收藏页、作者作品页批量同步；单链接可自由组合 `native`、`CDN` 与 `Kukutool` 解析。
...
- 清晰度：单链接和批量预览按文件大小从大到小展示并标记来源；同步下载所有已选来源中最大的可下载视频。Bilibili 优先下载最佳视频流与音频流并合并为 MP4。

## 使用流程

### 1. 选择平台工作台

- 抖音工作台显示 `native`、`CDN 原始高码率` 和 `Kukutool` 解析来源选择，默认全部启用。
...
`CDN 原始高码率` 使用抖音详情的内部视频 URI 请求 `ratio=default`，取得重定向后的临时 `douyinvod.com` 地址和文件大小；该地址会过期，因此解析后应尽快下载。该来源不需要打开 Kukutool 页面。
