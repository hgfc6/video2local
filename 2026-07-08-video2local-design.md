# Video2Local 设计文档

## 目标

构建一个桌面应用程序：由程序启动一个专用 Chrome 配置目录，你手动登录视频平台并打开某个可同步内容源页面，程序读取页面中的视频列表，把视频以当前可获取到的最高清晰度下载到本地，并按平台和作者自动归档。

第一版只支持抖音，但整体设计必须把平台相关逻辑和来源类型逻辑隔离开，后续可以在不重写核心系统的前提下扩展到哔哩哔哩、快手、YouTube 等平台，以及收藏页、作者作品页、播放列表等不同来源类型。

## 已确认的产品决策

- 交付形态：桌面应用
- 浏览器：由程序管理的专用 Chrome 窗口
- 首个平台：抖音
- 第一版来源类型：收藏页、作者作品页
- 下载内核：`yt-dlp`
- 清晰度目标：下载当前来源和 `yt-dlp` 能获取到的最高画质
- 归档目录：`platform/author_name/video_title [video_id].ext`
- 去重键：`platform + video_id`
- 持久化索引：SQLite
- 重复项处理：后续同步时跳过已下载视频

## 第一版非目标

- 不支持接管任意已经打开的 Chrome 窗口
- 不做浏览器扩展
- 不做全自动登录流程
- 第一阶段不支持多平台同时批量导出
- 不做转码、字幕抽取、封面归档等高级媒体处理，除非 `yt-dlp` 的输出处理必须涉及

## 总体架构

应用分为六个职责清晰的模块。

### 1. 桌面界面

界面负责承接用户操作，并实时展示运行状态。

主要操作：

- 启动专用 Chrome
- 校验当前页面
- 开始同步
- 停止同步
- 打开下载目录
- 查看最近日志或最近一次同步摘要

运行时反馈：

- 当前平台
- 当前作者
- 当前视频标题
- 当前执行阶段
- 进度计数：已发现、已下载、已跳过、已失败
- 当前项目的错误信息

### 2. Chrome 会话管理器

该模块负责以应用专用的用户数据目录启动 Chrome。该目录会长期保留，因此你只需要登录一次，后续就可以反复复用该会话。

职责：

- 定位 Chrome 可执行文件
- 使用专用 profile 路径启动 Chrome
- 让自动化层重新连接到该 Chrome 实例
- 向站点适配器暴露当前页面上下文

程序不需要控制你平时使用的默认 Chrome 配置。隔离专用目录是有意为之，因为它更稳定，也更容易支持。

### 3. 站点适配器层

每个平台实现同一套接口，把平台页面结构差异封装在内部。

初始接口：

- `can_handle_page(url: str) -> bool`
- `collect_video_candidates(page) -> Iterable[VideoCandidate]`
- `resolve_video_metadata(page, candidate) -> VideoMetadata`
- `normalize_author_name(raw_name: str) -> str`

第一版只提供 `DouyinAdapter`。

除平台适配外，系统还需要显式区分“来源类型”。来源类型描述当前页面属于哪一类可同步入口，而不直接等同于平台本身。

第一版支持的来源类型：

- `favorites`：收藏页
- `author_videos`：作者作品页

后续可扩展的来源类型：

- `playlist`：播放列表或合集
- `channel_uploads`：频道上传页
- `liked_videos`：点赞页

该层负责：

- 识别当前页面是否为受支持的内容源页面
- 滚动页面并收集候选视频
- 在需要时打开或解析实际视频页
- 抽取核心元数据，供系统其他部分使用

系统其他层不能依赖任何抖音专用的选择器或 URL 规则，也不能把“收藏页”写死为唯一内容来源。

### 4. 下载服务

该模块对 `yt-dlp` 做统一封装，而不是把站点专用下载逻辑直接写进应用里。

职责：

- 组装 `yt-dlp` 命令
- 请求最高可获取画质
- 捕获 stdout、stderr、退出码和输出文件路径
- 把结构化下载结果回传给同步引擎

应用把 `yt-dlp` 视为下载事实来源。如果平台实际只能暴露出“当前可获取的最高画质”，即便低于理论原画，程序仍然视为成功，因为第一版的契约是“在当前登录态和当前工具能力下拿到最佳可用版本”。

### 5. 归档管理器

该模块根据视频元数据生成确定性的本地路径，并保证文件名在 Windows 下安全可用。

规则：

- 根目录可配置
- 平台目录统一规范化，例如 `douyin`
- 作者目录由平台元数据规范化后生成
- 最终文件名格式为 `title [video_id].ext`
- 非法文件名字符会被移除或替换
- 如果标题为空，则回退为 `[video_id].ext`

即使 SQLite 中也会保存视频 ID，文件名里仍然保留 `[video_id]`，这样更利于人工识别，也能避免同标题覆盖。

### 6. 同步引擎

该模块负责串联整个同步流程。

单个视频的处理流程：

1. 站点适配器发现候选视频
2. 解析元数据，拿到稳定 `video_id`
3. 查询 SQLite 中是否已有 `(platform, video_id)`
4. 如果已经下载过，则标记为跳过
5. 否则调用 `yt-dlp` 下载
6. 将文件直接写入或整理到归档路径
7. 将结果写入 SQLite
8. 把进度推送到界面

同步引擎还负责取消、中断统计，以及中途停止后的安全续跑能力。

## 核心数据模型

至少需要两张持久化数据表。

### `videos`

这是媒体清单和去重的主表。

必需字段：

- `id`：整数主键
- `platform`：文本，非空
- `video_id`：文本，非空
- `author_name`：文本，非空
- `title`：文本，可空
- `source_url`：文本，非空
- `page_url`：文本，可空
- `local_path`：文本，可空
- `file_ext`：文本，可空
- `download_status`：文本，非空
- `downloaded_at`：时间，可空
- `file_size`：整数，可空
- `duration_seconds`：整数，可空
- `error_message`：文本，可空
- `created_at`：时间，非空
- `updated_at`：时间，非空

唯一约束：

- `(platform, video_id)`

建议状态值：

- `pending`
- `downloaded`
- `skipped_existing`
- `failed`

### `sync_runs`

该表记录一次完整同步任务。

必需字段：

- `id`：整数主键
- `platform`：文本，非空
- `started_at`：时间，非空
- `ended_at`：时间，可空
- `status`：文本，非空
- `discovered_count`：整数，非空，默认 0
- `downloaded_count`：整数，非空，默认 0
- `skipped_count`：整数，非空，默认 0
- `failed_count`：整数，非空，默认 0
- `error_message`：文本，可空

建议任务状态值：

- `running`
- `completed`
- `stopped`
- `failed`

## 同步流程

### 第一步：启动专用 Chrome

你在界面点击启动按钮，程序使用受管理的 profile 目录启动 Chrome。

### 第二步：手动登录并打开可同步内容源页面

你在该 Chrome 窗口中手动登录抖音，并打开某个受支持的页面，例如收藏页或作者作品页。这样可以避开脆弱的自动登录逻辑，也最符合你的真实使用方式。

### 第三步：校验页面

在开始同步之前，当前适配器先判断当前页是否为受支持的内容源页面，并识别它的来源类型。如果不是，界面阻止任务开始，并给出明确提示。

### 第四步：增量发现候选视频

适配器采用“边滚动边采集”的方式逐步收集候选视频，而不是要求先把整页收藏一次性全部解析出来。这样更省内存，也能降低无限滚动页面不稳定带来的风险。

### 第五步：解析元数据

对每个候选视频，适配器至少要解析出：

- 平台名
- 稳定视频 ID
- 标题
- 作者名
- 来源页面 URL
- 可交给 `yt-dlp` 的下载页面 URL 或媒体目标

同步引擎只有在拿到稳定 `video_id` 后，才能做真正的去重判断。

### 第六步：去重

同步引擎按 `(platform, video_id)` 查询 SQLite。

- 如果已经存在成功下载记录，则跳过
- 如果存在失败记录，是否自动重试可以在后续版本增强，第一版先保守处理
- 如果不存在，则继续下载

### 第七步：下载

程序用当前平台所需的已登录上下文调用 `yt-dlp`。不同平台的命令细节可以不同，但统一目标都是下载最高可获取画质。

### 第八步：归档落盘

下载完成的媒体文件直接写入或移动到：

`<download_root>/<platform>/<author_name>/<title> [<video_id>].<ext>`

### 第九步：持久化与进度更新

程序更新数据库记录，并刷新界面统计。单个视频失败时只记录错误，不中断整个同步任务。

## 抖音第一版适配器要求

`DouyinAdapter` 至少要支持这些能力：

- 判断当前页面是否为当前登录账号的收藏页或某个作者作品页
- 返回当前页面的来源类型
- 在滚动过程中发现单个视频条目
- 在真正去重前解析出稳定的抖音视频 ID
- 足够可靠地提取作者名和标题，用于归档命名
- 向 `yt-dlp` 提供可用的视频页地址或下载目标

对于作者作品页，适配器还要能识别当前作者上下文，避免把页面所属作者与视频发布作者混淆。

它还必须容忍这些真实情况：

- 懒加载列表
- 偶尔缺失标题
- 滚动时重复出现的卡片
- 临时性的页面时序波动

适配器可以在单次运行内先做一层内存去重，但最终仍以 SQLite 作为唯一真相来源。

## 错误处理策略

第一版优先追求“不中断整批任务”，而不是追求极端完美。

### 阻断性错误

这些错误会直接阻止同步开始：

- Chrome 启动失败
- 自动化层无法连接到专用 Chrome
- 当前页面不是支持的内容源页面
- SQLite 无法打开
- `yt-dlp` 缺失或不可用

### 单项错误

这些错误只会导致当前视频失败，不会终止整次同步：

- 元数据解析失败
- 视频页面失效
- 登录态过期
- `yt-dlp` 返回错误
- 文件移动或最终写入失败

失败时，程序把可获取到的最佳错误信息写入 `videos.error_message`，然后继续处理下一个视频。

## 停止与恢复

你可以在界面中手动停止同步任务。

停止行为：

- 引擎停止接收新的处理项
- 当前视频会在一个安全边界处结束
- 当前任务状态记为 `stopped`

第一版不单独做复杂断点续传机制，而是采用“重新发起一次同步 + SQLite 去重跳过已完成项”的方式恢复。这已经足够支撑第一版，也避免过早引入额外复杂度。

## 文件系统规则

必须统一处理 Windows 文件命名约束。

规范化要求：

- 替换非法字符，例如 `<>:"/\\|?*`
- 去除结尾空格和句点
- 合并多余空白字符
- 尽量保持可读性，不使用纯哈希命名

即使两个视频标题完全相同，文件名中的 `[video_id]` 也能保证最终路径不冲突。

## 扩展方案

除了适配器层之外，核心应用应该尽量平台无关。与此同时，适配器内部也不应该把单一来源类型写死。

未来新增哔哩哔哩、快手、YouTube 时：

- 复用同一套桌面界面
- 复用同一套 SQLite 结构
- 复用同一套归档管理器
- 复用同一套同步引擎
- 为对应平台新增适配器实现，并按需要补充来源类型解析

该设计的核心原则是：把“如何识别和读取内容源”与“如何下载、存储、追踪媒体”彻底分开。

## 建议技术形态

具体语言仍可在实现计划阶段最终确定，但整体技术结构需要满足：

- 桌面界面能力
- Chrome 自动化能力
- SQLite 访问能力
- 调用 `yt-dlp` 的进程执行能力
- 即使第一版偏 Windows，也应具备跨平台安全的路径处理能力

如果尽量保持单语言实现，Python 是一个很务实的起点，因为它在桌面工具、浏览器自动化、SQLite 和进程编排上都比较成熟。

## 测试策略

第一版的自动化测试应主要覆盖那些稳定、值得测的边界。

适合自动化测试的部分：

- 文件名规范化
- 归档路径生成
- SQLite 去重逻辑
- 同步引擎对下载成功、跳过、失败的状态流转
- 使用伪造适配器输出时的适配器无关编排逻辑

应尽量减少或隔离的测试：

- 直接依赖真实抖音页面的完整端到端单元测试
- 强依赖时间等待的脆弱 UI 自动化测试

由于真实平台页面天然不稳定，第一版仍然需要少量人工集成验证，且至少要覆盖抖音收藏页和抖音作者作品页这两种来源类型。

## 待实现阶段确定的工程决策

这些问题会在实现计划中定稿，但不会影响当前产品设计本身：

- 具体桌面框架选择
- 具体 Chrome 自动化库选择
- `yt-dlp` 的具体参数和登录态传递方式
- 日志格式和日志文件位置
- 失败记录是在下一次同步时自动重试，还是只在显式操作时重试

产品行为边界已经由本设计固定，剩下的是工程实现细节。
